...
class VitalsData(BaseModel):
    ...
    quality: Optional[int] = Field(None, ge=0, le=100, description="Bridge-assigned vitals quality score")
...
class MetadataInfo(BaseModel):
    ...
    schema_version: Optional[str] = Field(None, description="Firmware-side payload schema version")
    bridge_quality: Optional[int] = Field(None, ge=0, le=100, description="Overall bridge quality score")
    bridge_fresh: Optional[bool] = Field(None, description="Whether bridge data is fresh")
    c3_online: Optional[bool] = Field(None, description="Whether the C3 board is online")
    mpu_online: Optional[bool] = Field(None, description="Whether the MPU sensor is online")
    sensor_state: Optional[str] = Field(None, description="Firmware-reported sensor state")
    device_name: Optional[str] = Field(None, description="Firmware-reported device display name")
    sensor_id: Optional[str] = Field(None, description="Firmware-reported sensor identifier")
...
class ECGData(BaseModel):
    ...
    window_seconds: Optional[float] = Field(None, description="Capture window duration in seconds")
    sample_count: Optional[int] = Field(None, ge=1, description="Number of samples in this batch")
    captured_at_ms: Optional[int] = Field(None, ge=0, description="Firmware-side capture timestamp in milliseconds")
...
class HealthReading(BaseModel):
    ...
    payload_type: Optional[str] = Field(default=None, description="vitals, ecg_batch, fall_alert, ...")
    ...
    fall_phase: Optional[str] = Field(
        None,
        max_length=64,
        validation_alias=AliasChoices("fall_phase", "fall_state"),
        description="Firmware-reported fall state",
    )
